(function () {
  ...
  function getFormData() {
    const data = {
      ...
      template: (document.querySelector('input[name="resume_template"]:checked') || {}).value || 'classic',
  ...
  function buildPreviewHtml(d) {
    ...
    const layout = d.template || 'classic';
    ...
    function section(title, body) {
      return '<div class="resume-section"><div class="resume-section-title">' + title + '</div>' + body + '</div>';
    }
    var summaryBlock = d.summary ? section('Summary', '<div class="resume-summary">' + escapeHtml(d.summary) + '</div>') : '';
    var langList = d.languages ? d.languages.split(',').map(function (l) { return l.trim(); }).filter(Boolean) : [];
    var languagesBlock = langList.length ? section('Languages', '<div class="resume-item-desc">' + langList.map(function (l) { return escapeHtml(l); }).join(' • ') + '</div>') : '';
    var educationBlock = '';
    if (d.education && d.education.length) {
      educationBlock = '<div class="resume-section"><div class="resume-section-title">Education</div>';
      d.education.forEach(function (edu) {
        educationBlock += '<div class="resume-item"><div class="resume-item-title">' + escapeHtml(edu.degree) + '</div><div class="resume-item-meta">' + escapeHtml(edu.institution) + (edu.year ? ' | ' + escapeHtml(edu.year) : '') + (edu.grade ? ' | ' + escapeHtml(edu.grade) : '') + '</div></div>';
      });
      educationBlock += '</div>';
    }
    var experienceBlock = '';
    if (d.experience && d.experience.length) {
      experienceBlock = '<div class="resume-section"><div class="resume-section-title">Experience / Internships</div>';
      d.experience.forEach(function (exp) {
        var meta = [exp.duration, exp.location].filter(Boolean).map(escapeHtml).join(' | ');
        experienceBlock += '<div class="resume-item"><div class="resume-item-title">' + escapeHtml(exp.title) + (exp.company ? ' — ' + escapeHtml(exp.company) : '') + '</div>' + (meta ? '<div class="resume-item-meta">' + meta + '</div>' : '') + (exp.desc ? '<div class="resume-item-desc">' + escapeHtml(exp.desc) + '</div>' : '') + '</div>';
      });
      experienceBlock += '</div>';
    }
    var skillList = d.skills ? d.skills.split(',').map(function (s) { return s.trim(); }).filter(Boolean) : [];
    var skillsBlock = '';
    if (skillList.length) {
      skillsBlock = '<div class="resume-section"><div class="resume-section-title">Skills</div><div class="resume-skills">';
      skillList.forEach(function (s) { skillsBlock += '<span>' + escapeHtml(s) + '</span>'; });
      skillsBlock += '</div></div>';
    }
    var projectsBlock = '';
    if ((industry === 'cs' || industry === 'engineering') && d.projects && d.projects.length) {
      projectsBlock = '<div class="resume-section"><div class="resume-section-title">Projects</div>';
      d.projects.forEach(function (p) {
        projectsBlock += '<div class="resume-item"><div class="resume-item-title">' + escapeHtml(p.name) + '</div>' + (p.desc ? '<div class="resume-item-desc">' + escapeHtml(p.desc) + '</div>' : '') + '</div>';
      });
      projectsBlock += '</div>';
    }
    var certList = d.certifications ? d.certifications.split(',').map(function (c) { return c.trim(); }).filter(Boolean) : [];
    var certsBlock = certList.length ? section('Certifications', '<div class="resume-item-desc">' + certList.map(function (c) { return escapeHtml(c); }).join(' • ') + '</div>') : '';
    var farmBlock = (industry === 'agriculture' && d.farm_exp) ? section('Farm / Field Experience', '<div class="resume-summary">' + escapeHtml(d.farm_exp) + '</div>') : '';
    var leadershipBlock = (industry === 'management' && d.leadership) ? section('Leadership / Achievements', '<div class="resume-summary">' + escapeHtml(d.leadership) + '</div>') : '';
    var refsBlock = '';
    if (d.ref_available || (d.references && d.references.trim())) {
      refsBlock = section('References', (d.references && d.references.trim()) ? '<div class="resume-item-desc">' + escapeHtml(d.references) + '</div>' : '<div class="resume-item-desc">References available on request.</div>');
    }

    if (layout === 'sidebar') {
      var contactSidebar = contactParts.length ? contactParts.join('<br>') : '';
      var leftHtml = '<div class="resume-sidebar-left"><div class="resume-name">' + escapeHtml(d.fullname || 'Your Name') + '</div><div class="resume-contact">' + contactSidebar + '</div>' + skillsBlock + languagesBlock + certsBlock + '</div>';
      var rightHtml = '<div class="resume-sidebar-right">' + summaryBlock + educationBlock + experienceBlock + projectsBlock + farmBlock + leadershipBlock + refsBlock + '</div>';
      return '<div class="resume-paper-inner ' + templateClass + ' layout-sidebar"><div class="resume-sidebar-wrap">' + leftHtml + rightHtml + '</div></div>';
    }

    var html = '<div class="resume-paper-inner ' + templateClass + ' layout-' + layout + '">';
    html += '<div class="resume-name">' + escapeHtml(d.fullname || 'Your Name') + '</div>';
    html += '<div class="resume-contact">' + contactHtml + '</div>';
    html += summaryBlock + languagesBlock + educationBlock + experienceBlock + skillsBlock + projectsBlock + certsBlock + farmBlock + leadershipBlock + refsBlock;
    html += '</div>';
    return html;
  }
  ...
  function updatePreview() {
    ...
    preview.className = 'resume-paper template-' + (data.industry || 'general') + ' layout-' + (data.template || 'classic');
